...
describe('Offline Support', () => {
  describe('fetchWithRetry', () => {
    ...
    it('should retry on network failure', async () => {
      ...
      const response = await fetchWithRetry('http://localhost:4000/api/test', {}, { maxRetries: 1, initialDelayMs: 0 });
    ...
    it('should fail after max retries exhausted', async () => {
      ...
      await expect(
        fetchWithRetry('http://localhost:4000/api/test', {}, { maxRetries: 2, initialDelayMs: 0 })
    ...
    it('should not retry on 400/401/403/404 errors', async () => {
      ...
      const response = await fetchWithRetry('http://localhost:4000/api/test', {}, { maxRetries: 3, initialDelayMs: 0 });
    ...
    it('should handle timeout correctly', async () => {
      ...
      const response = await fetchWithRetry('http://localhost:4000/api/test', { timeout: 100 }, { maxRetries: 0 });
    ...
    it('should apply exponential backoff', async () => {
      ...
      await fetchWithRetry('http://localhost:4000/api/test', {}, { maxRetries: 2, initialDelayMs: 100, backoffMultiplier: 2 });
